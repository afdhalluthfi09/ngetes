 ...
 * Page class
 */
class ProdukTidakLolosKurasiList extends ProdukTidakLolosKurasi
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'Produk Tidak Lolos Kurasi';

    // Page object name
    public $PageObjName = "ProdukTidakLolosKurasiList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fProduk_Tidak_Lolos_Kurasilist";
    ...
    public function __construct()
    {
        ...
        // Table object (Produk_Tidak_Lolos_Kurasi)
        if (!isset($GLOBALS["Produk_Tidak_Lolos_Kurasi"]) || get_class($GLOBALS["Produk_Tidak_Lolos_Kurasi"]) == PROJECT_NAMESPACE . "Produk_Tidak_Lolos_Kurasi") {
            $GLOBALS["Produk_Tidak_Lolos_Kurasi"] = &$this;
        }
        ...
        $this->AddUrl = "produktidakloloskurasiadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "produktidakloloskurasidelete";
        $this->MultiUpdateUrl = "produktidakloloskurasiupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Produk Tidak Lolos Kurasi');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fProduk_Tidak_Lolos_Kurasilistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("Produk_Tidak_Lolos_Kurasi"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fProduk_Tidak_Lolos_Kurasilistsrch");
        }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fProduk_Tidak_Lolos_Kurasilistsrch", $filters);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fProduk_Tidak_Lolos_Kurasilistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fProduk_Tidak_Lolos_Kurasilistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fProduk_Tidak_Lolos_Kurasilist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->produk_foto_1->UseColorbox) {
                ...
                $this->produk_foto_1->LinkAttrs["data-rel"] = "Produk_Tidak_Lolos_Kurasi_x" . $this->RowCount . "_produk_foto_1";
            ...
            if ($this->produk_foto_2->UseColorbox) {
                ...
                $this->produk_foto_2->LinkAttrs["data-rel"] = "Produk_Tidak_Lolos_Kurasi_x" . $this->RowCount . "_produk_foto_2";
            ...
            if ($this->produk_foto_3->UseColorbox) {
                ...
                $this->produk_foto_3->LinkAttrs["data-rel"] = "Produk_Tidak_Lolos_Kurasi_x" . $this->RowCount . "_produk_foto_3";
