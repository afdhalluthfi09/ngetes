 ...
 * Page class
 */
class UmkmDatadiriList extends UmkmDatadiri
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UmkmDatadiriList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fumkm_datadirilist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "umkmdatadiriadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "umkmdatadiridelete";
        $this->MultiUpdateUrl = "umkmdatadiriupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fumkm_datadirilistsrch";
    ...
    public $PageSizes = "100,1000,10000"; // Page sizes (comma separated)
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->NAMA_PEMILIK, $ctrl); // NAMA_PEMILIK
            $this->updateSort($this->JENIS_KELAMIN, $ctrl); // JENIS_KELAMIN
            $this->updateSort($this->NO_HP, $ctrl); // NO_HP
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_umkm_datausaha"
        $item = &$this->ListOptions->add("detail_umkm_datausaha");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'umkm_datausaha') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
        $item->ShowInButtonGroup = false;

        // "detail_umkm_aspekkeuangan"
        $item = &$this->ListOptions->add("detail_umkm_aspekkeuangan");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'umkm_aspekkeuangan') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
        $item->ShowInButtonGroup = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("umkm_datausaha");
        $pages->add("umkm_aspekkeuangan");
    ...
    public function renderListOptions()
    {
        ...
        // "detail_umkm_datausaha"
        $opt = $this->ListOptions["detail_umkm_datausaha"];
        if ($Security->allowList(CurrentProjectID() . 'umkm_datausaha')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("umkm_datausaha", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("umkmdatausahalist?" . Config("TABLE_SHOW_MASTER") . "=umkm_datadiri&" . GetForeignKeyUrl("fk_NIK", $this->NIK->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("UmkmDatausahaGrid");
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'umkm_datadiri')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=umkm_datausaha");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "umkm_datausaha";
            }
            if ($links != "") {
        ...
        // "detail_umkm_aspekkeuangan"
        $opt = $this->ListOptions["detail_umkm_aspekkeuangan"];
        if ($Security->allowList(CurrentProjectID() . 'umkm_aspekkeuangan')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("umkm_aspekkeuangan", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("umkmaspekkeuanganlist?" . Config("TABLE_SHOW_MASTER") . "=umkm_datadiri&" . GetForeignKeyUrl("fk_NIK", $this->NIK->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("UmkmAspekkeuanganGrid");
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'umkm_datadiri')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=umkm_aspekkeuangan");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "umkm_aspekkeuangan";
            }
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fumkm_datadirilist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'delete'}, msg:ew.language.phrase('DeleteConfirmMsg')});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fumkm_datadirilistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fumkm_datadirilistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fumkm_datadirilist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
