 ...
 * Page class
 */
class BinapesertalengkapList extends Binapesertalengkap
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'binapesertalengkap';

    // Page object name
    public $PageObjName = "BinapesertalengkapList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fbinapesertalengkaplist";
    ...
    public function __construct()
    {
        ...
        // Table object (binapesertalengkap)
        if (!isset($GLOBALS["binapesertalengkap"]) || get_class($GLOBALS["binapesertalengkap"]) == PROJECT_NAMESPACE . "binapesertalengkap") {
            $GLOBALS["binapesertalengkap"] = &$this;
        }
        ...
        $this->AddUrl = "binapesertalengkapadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "binapesertalengkapdelete";
        $this->MultiUpdateUrl = "binapesertalengkapupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'binapesertalengkap');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fbinapesertalengkaplistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("binapesertalengkap"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->id->Visible = false;
        $this->id_binadata->Visible = false;
        $this->idperiode->Visible = false;
        $this->periode_tahun->setVisibility();
        $this->periode_bulan->setVisibility();
        $this->idkelompok->Visible = false;
        $this->kelompok_pembinaan->setVisibility();
        $this->namakegiatan->setVisibility();
        $this->uraian->Visible = false;
        $this->tglmulai->Visible = false;
        $this->tglakhir->Visible = false;
        $this->narasumber->Visible = false;
        $this->kontak_nama->Visible = false;
        $this->kontak_hp->Visible = false;
        $this->poster->Visible = false;
        $this->postertipe->Visible = false;
        $this->posterukuran->Visible = false;
        $this->posterlebar->Visible = false;
        $this->postertinggi->Visible = false;
        $this->linkinfo->Visible = false;
        $this->peserta_kelas->Visible = false;
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`periode_tahun` DESC,`periode_bulan` DESC,`id` DESC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->periode_tahun->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($this->periode_bulan->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($this->id->getSort() != "") {
                    $useDefaultSort = false;
                ...
                if ($useDefaultSort) {
                    $this->periode_tahun->setSort("DESC");
                    $this->periode_bulan->setSort("DESC");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbinapesertalengkaplistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbinapesertalengkaplistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fbinapesertalengkaplist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fbinapesertalengkaplist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fbinapesertalengkaplist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fbinapesertalengkaplist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_binapesertalengkap" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_binapesertalengkap\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fbinapesertalengkaplist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
