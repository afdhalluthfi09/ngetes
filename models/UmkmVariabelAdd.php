 ...
 * Page class
 */
class UmkmVariabelAdd extends UmkmVariabel
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UmkmVariabelAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "umkmvariabelview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->variabel->CurrentValue = null;
        $this->variabel->OldValue = $this->variabel->CurrentValue;
        $this->nmin->CurrentValue = 0;
        $this->nmax->CurrentValue = 0;
        $this->subkat->CurrentValue = null;
        $this->subkat->OldValue = $this->subkat->CurrentValue;
        $this->bobot->CurrentValue = 0;
        $this->kat->CurrentValue = null;
        $this->kat->OldValue = $this->kat->CurrentValue;
        $this->porsi->CurrentValue = 0;
    }
    ...
    protected function newRow()
    {
        ...
        $row['variabel'] = $this->variabel->CurrentValue;
        $row['nmin'] = $this->nmin->CurrentValue;
        $row['nmax'] = $this->nmax->CurrentValue;
        $row['subkat'] = $this->subkat->CurrentValue;
        $row['bobot'] = $this->bobot->CurrentValue;
        $row['kat'] = $this->kat->CurrentValue;
        $row['porsi'] = $this->porsi->CurrentValue;
        return $row;
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // variabel
        $this->variabel->setDbValueDef($rsnew, $this->variabel->CurrentValue, null, false);

        // nmin
        $this->nmin->setDbValueDef($rsnew, $this->nmin->CurrentValue, null, strval($this->nmin->CurrentValue) == "");

        // nmax
        $this->nmax->setDbValueDef($rsnew, $this->nmax->CurrentValue, null, strval($this->nmax->CurrentValue) == "");

        // subkat
        $this->subkat->setDbValueDef($rsnew, $this->subkat->CurrentValue, null, false);

        // bobot
        $this->bobot->setDbValueDef($rsnew, $this->bobot->CurrentValue, null, strval($this->bobot->CurrentValue) == "");

        // kat
        $this->kat->setDbValueDef($rsnew, $this->kat->CurrentValue, null, false);

        // porsi
        $this->porsi->setDbValueDef($rsnew, $this->porsi->CurrentValue, null, strval($this->porsi->CurrentValue) == "");
