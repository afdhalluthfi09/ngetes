 ...
 * Page class
 */
class VUmkmDatadiriEdit extends VUmkmDatadiri
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_umkm_datadiri';

    // Page object name
    public $PageObjName = "VUmkmDatadiriEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (v_umkm_datadiri)
        if (!isset($GLOBALS["v_umkm_datadiri"]) || get_class($GLOBALS["v_umkm_datadiri"]) == PROJECT_NAMESPACE . "v_umkm_datadiri") {
            $GLOBALS["v_umkm_datadiri"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_umkm_datadiri');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_umkm_datadiri"));
                $doc->Text = @$content;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("vumkmdatadirilist"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "vumkmdatadirilist") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'IDSIDAKUI' first before field var 'x_IDSIDAKUI'
        $val = $CurrentForm->hasValue("IDSIDAKUI") ? $CurrentForm->getValue("IDSIDAKUI") : $CurrentForm->getValue("x_IDSIDAKUI");
        if (!$this->IDSIDAKUI->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->IDSIDAKUI->Visible = false; // Disable update for API request
            } else {
                $this->IDSIDAKUI->setFormValue($val);
            }
        }

        // Check field name 'NoKTP' first before field var 'x_NoKTP'
        $val = $CurrentForm->hasValue("NoKTP") ? $CurrentForm->getValue("NoKTP") : $CurrentForm->getValue("x_NoKTP");
        if (!$this->NoKTP->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->NoKTP->Visible = false; // Disable update for API request
            } else {
                $this->NoKTP->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->IDSIDAKUI->CurrentValue = $this->IDSIDAKUI->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->IDSIDAKUI->EditValue = HtmlEncode($this->IDSIDAKUI->CurrentValue);
            ...
            $this->NoKTP->EditValue = HtmlEncode($this->NoKTP->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->IDSIDAKUI->Required) {
            if (!$this->IDSIDAKUI->IsDetailKey && EmptyValue($this->IDSIDAKUI->FormValue)) {
                $this->IDSIDAKUI->addErrorMessage(str_replace("%s", $this->IDSIDAKUI->caption(), $this->IDSIDAKUI->RequiredErrorMessage));
            }
        }
        if ($this->NoKTP->Required) {
            if (!$this->NoKTP->IsDetailKey && EmptyValue($this->NoKTP->FormValue)) {
                $this->NoKTP->addErrorMessage(str_replace("%s", $this->NoKTP->caption(), $this->NoKTP->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // IDSIDAKUI
            $this->IDSIDAKUI->setDbValueDef($rsnew, $this->IDSIDAKUI->CurrentValue, "", $this->IDSIDAKUI->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("vumkmdatadirilist"), "", $this->TableVar, true);
        $pageId = "edit";
