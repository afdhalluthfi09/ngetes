 ...
 * Page class
 */
class PhpKurasiLolosList extends PhpKurasiLolos
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'php_kurasi_lolos';

    // Page object name
    public $PageObjName = "PhpKurasiLolosList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fphp_kurasi_loloslist";
    ...
    public function __construct()
    {
        ...
        // Table object (php_kurasi_lolos)
        if (!isset($GLOBALS["php_kurasi_lolos"]) || get_class($GLOBALS["php_kurasi_lolos"]) == PROJECT_NAMESPACE . "php_kurasi_lolos") {
            $GLOBALS["php_kurasi_lolos"] = &$this;
        }
        ...
        $this->AddUrl = "phpkurasilolosadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "phpkurasilolosdelete";
        $this->MultiUpdateUrl = "phpkurasilolosupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'php_kurasi_lolos');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fphp_kurasi_loloslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("php_kurasi_lolos"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fphp_kurasi_loloslistsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->nik_ukm->AdvancedSearch->toJson(), ","); // Field nik_ukm
        ...
        $filterList = Concat($filterList, $this->produk_legal->AdvancedSearch->toJson(), ","); // Field produk_legal
        $filterList = Concat($filterList, $this->judul_sesuai->AdvancedSearch->toJson(), ","); // Field judul_sesuai
        $filterList = Concat($filterList, $this->foto_bagus->AdvancedSearch->toJson(), ","); // Field foto_bagus
        $filterList = Concat($filterList, $this->deskripsi_jelas->AdvancedSearch->toJson(), ","); // Field deskripsi_jelas
        $filterList = Concat($filterList, $this->harga_tidak_kosong->AdvancedSearch->toJson(), ","); // Field harga_tidak_kosong
        $filterList = Concat($filterList, $this->berat_tidak_kosong->AdvancedSearch->toJson(), ","); // Field berat_tidak_kosong
        $filterList = Concat($filterList, $this->kurasi->AdvancedSearch->toJson(), ","); // Field kurasi
        $filterList = Concat($filterList, $this->waktu_entry->AdvancedSearch->toJson(), ","); // Field waktu_entry
        $filterList = Concat($filterList, $this->waktu_kurasi->AdvancedSearch->toJson(), ","); // Field waktu_kurasi
        $filterList = Concat($filterList, $this->waktu_update->AdvancedSearch->toJson(), ","); // Field waktu_update
        $filterList = Concat($filterList, $this->editor->AdvancedSearch->toJson(), ","); // Field editor
        ...
        $filterList = Concat($filterList, $this->produk_foto_1->AdvancedSearch->toJson(), ","); // Field produk_foto_1
        $filterList = Concat($filterList, $this->produk_foto_2->AdvancedSearch->toJson(), ","); // Field produk_foto_2
        $filterList = Concat($filterList, $this->produk_foto_3->AdvancedSearch->toJson(), ","); // Field produk_foto_3
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fphp_kurasi_loloslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field nik_ukm
        $this->nik_ukm->AdvancedSearch->SearchValue = @$filter["x_nik_ukm"];
        $this->nik_ukm->AdvancedSearch->SearchOperator = @$filter["z_nik_ukm"];
        $this->nik_ukm->AdvancedSearch->SearchCondition = @$filter["v_nik_ukm"];
        $this->nik_ukm->AdvancedSearch->SearchValue2 = @$filter["y_nik_ukm"];
        $this->nik_ukm->AdvancedSearch->SearchOperator2 = @$filter["w_nik_ukm"];
        $this->nik_ukm->AdvancedSearch->save();
        ...
        // Field produk_legal
        $this->produk_legal->AdvancedSearch->SearchValue = @$filter["x_produk_legal"];
        $this->produk_legal->AdvancedSearch->SearchOperator = @$filter["z_produk_legal"];
        $this->produk_legal->AdvancedSearch->SearchCondition = @$filter["v_produk_legal"];
        $this->produk_legal->AdvancedSearch->SearchValue2 = @$filter["y_produk_legal"];
        $this->produk_legal->AdvancedSearch->SearchOperator2 = @$filter["w_produk_legal"];
        $this->produk_legal->AdvancedSearch->save();

        // Field judul_sesuai
        $this->judul_sesuai->AdvancedSearch->SearchValue = @$filter["x_judul_sesuai"];
        $this->judul_sesuai->AdvancedSearch->SearchOperator = @$filter["z_judul_sesuai"];
        $this->judul_sesuai->AdvancedSearch->SearchCondition = @$filter["v_judul_sesuai"];
        $this->judul_sesuai->AdvancedSearch->SearchValue2 = @$filter["y_judul_sesuai"];
        $this->judul_sesuai->AdvancedSearch->SearchOperator2 = @$filter["w_judul_sesuai"];
        $this->judul_sesuai->AdvancedSearch->save();

        // Field foto_bagus
        $this->foto_bagus->AdvancedSearch->SearchValue = @$filter["x_foto_bagus"];
        $this->foto_bagus->AdvancedSearch->SearchOperator = @$filter["z_foto_bagus"];
        $this->foto_bagus->AdvancedSearch->SearchCondition = @$filter["v_foto_bagus"];
        $this->foto_bagus->AdvancedSearch->SearchValue2 = @$filter["y_foto_bagus"];
        $this->foto_bagus->AdvancedSearch->SearchOperator2 = @$filter["w_foto_bagus"];
        $this->foto_bagus->AdvancedSearch->save();

        // Field deskripsi_jelas
        $this->deskripsi_jelas->AdvancedSearch->SearchValue = @$filter["x_deskripsi_jelas"];
        $this->deskripsi_jelas->AdvancedSearch->SearchOperator = @$filter["z_deskripsi_jelas"];
        $this->deskripsi_jelas->AdvancedSearch->SearchCondition = @$filter["v_deskripsi_jelas"];
        $this->deskripsi_jelas->AdvancedSearch->SearchValue2 = @$filter["y_deskripsi_jelas"];
        $this->deskripsi_jelas->AdvancedSearch->SearchOperator2 = @$filter["w_deskripsi_jelas"];
        $this->deskripsi_jelas->AdvancedSearch->save();

        // Field harga_tidak_kosong
        $this->harga_tidak_kosong->AdvancedSearch->SearchValue = @$filter["x_harga_tidak_kosong"];
        $this->harga_tidak_kosong->AdvancedSearch->SearchOperator = @$filter["z_harga_tidak_kosong"];
        $this->harga_tidak_kosong->AdvancedSearch->SearchCondition = @$filter["v_harga_tidak_kosong"];
        $this->harga_tidak_kosong->AdvancedSearch->SearchValue2 = @$filter["y_harga_tidak_kosong"];
        $this->harga_tidak_kosong->AdvancedSearch->SearchOperator2 = @$filter["w_harga_tidak_kosong"];
        $this->harga_tidak_kosong->AdvancedSearch->save();

        // Field berat_tidak_kosong
        $this->berat_tidak_kosong->AdvancedSearch->SearchValue = @$filter["x_berat_tidak_kosong"];
        $this->berat_tidak_kosong->AdvancedSearch->SearchOperator = @$filter["z_berat_tidak_kosong"];
        $this->berat_tidak_kosong->AdvancedSearch->SearchCondition = @$filter["v_berat_tidak_kosong"];
        $this->berat_tidak_kosong->AdvancedSearch->SearchValue2 = @$filter["y_berat_tidak_kosong"];
        $this->berat_tidak_kosong->AdvancedSearch->SearchOperator2 = @$filter["w_berat_tidak_kosong"];
        $this->berat_tidak_kosong->AdvancedSearch->save();

        // Field kurasi
        $this->kurasi->AdvancedSearch->SearchValue = @$filter["x_kurasi"];
        $this->kurasi->AdvancedSearch->SearchOperator = @$filter["z_kurasi"];
        $this->kurasi->AdvancedSearch->SearchCondition = @$filter["v_kurasi"];
        $this->kurasi->AdvancedSearch->SearchValue2 = @$filter["y_kurasi"];
        $this->kurasi->AdvancedSearch->SearchOperator2 = @$filter["w_kurasi"];
        $this->kurasi->AdvancedSearch->save();

        // Field waktu_entry
        $this->waktu_entry->AdvancedSearch->SearchValue = @$filter["x_waktu_entry"];
        $this->waktu_entry->AdvancedSearch->SearchOperator = @$filter["z_waktu_entry"];
        $this->waktu_entry->AdvancedSearch->SearchCondition = @$filter["v_waktu_entry"];
        $this->waktu_entry->AdvancedSearch->SearchValue2 = @$filter["y_waktu_entry"];
        $this->waktu_entry->AdvancedSearch->SearchOperator2 = @$filter["w_waktu_entry"];
        $this->waktu_entry->AdvancedSearch->save();

        // Field waktu_kurasi
        $this->waktu_kurasi->AdvancedSearch->SearchValue = @$filter["x_waktu_kurasi"];
        $this->waktu_kurasi->AdvancedSearch->SearchOperator = @$filter["z_waktu_kurasi"];
        $this->waktu_kurasi->AdvancedSearch->SearchCondition = @$filter["v_waktu_kurasi"];
        $this->waktu_kurasi->AdvancedSearch->SearchValue2 = @$filter["y_waktu_kurasi"];
        $this->waktu_kurasi->AdvancedSearch->SearchOperator2 = @$filter["w_waktu_kurasi"];
        $this->waktu_kurasi->AdvancedSearch->save();

        // Field waktu_update
        $this->waktu_update->AdvancedSearch->SearchValue = @$filter["x_waktu_update"];
        $this->waktu_update->AdvancedSearch->SearchOperator = @$filter["z_waktu_update"];
        $this->waktu_update->AdvancedSearch->SearchCondition = @$filter["v_waktu_update"];
        $this->waktu_update->AdvancedSearch->SearchValue2 = @$filter["y_waktu_update"];
        $this->waktu_update->AdvancedSearch->SearchOperator2 = @$filter["w_waktu_update"];
        $this->waktu_update->AdvancedSearch->save();
        ...
        // Field produk_foto_1
        $this->produk_foto_1->AdvancedSearch->SearchValue = @$filter["x_produk_foto_1"];
        $this->produk_foto_1->AdvancedSearch->SearchOperator = @$filter["z_produk_foto_1"];
        $this->produk_foto_1->AdvancedSearch->SearchCondition = @$filter["v_produk_foto_1"];
        $this->produk_foto_1->AdvancedSearch->SearchValue2 = @$filter["y_produk_foto_1"];
        $this->produk_foto_1->AdvancedSearch->SearchOperator2 = @$filter["w_produk_foto_1"];
        $this->produk_foto_1->AdvancedSearch->save();

        // Field produk_foto_2
        $this->produk_foto_2->AdvancedSearch->SearchValue = @$filter["x_produk_foto_2"];
        $this->produk_foto_2->AdvancedSearch->SearchOperator = @$filter["z_produk_foto_2"];
        $this->produk_foto_2->AdvancedSearch->SearchCondition = @$filter["v_produk_foto_2"];
        $this->produk_foto_2->AdvancedSearch->SearchValue2 = @$filter["y_produk_foto_2"];
        $this->produk_foto_2->AdvancedSearch->SearchOperator2 = @$filter["w_produk_foto_2"];
        $this->produk_foto_2->AdvancedSearch->save();

        // Field produk_foto_3
        $this->produk_foto_3->AdvancedSearch->SearchValue = @$filter["x_produk_foto_3"];
        $this->produk_foto_3->AdvancedSearch->SearchOperator = @$filter["z_produk_foto_3"];
        $this->produk_foto_3->AdvancedSearch->SearchCondition = @$filter["v_produk_foto_3"];
        $this->produk_foto_3->AdvancedSearch->SearchValue2 = @$filter["y_produk_foto_3"];
        $this->produk_foto_3->AdvancedSearch->SearchOperator2 = @$filter["w_produk_foto_3"];
        $this->produk_foto_3->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->nik_ukm, $arKeywords, $type);
        ...
        $this->buildBasicSearchSql($where, $this->kurasi, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->editor, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->nik_ukm, $ctrl); // nik_ukm
            ...
            $this->updateSort($this->produk_legal, $ctrl); // produk_legal
            ...
            $this->updateSort($this->waktu_entry, $ctrl); // waktu_entry
            $this->updateSort($this->waktu_kurasi, $ctrl); // waktu_kurasi
            $this->updateSort($this->waktu_update, $ctrl); // waktu_update
            $this->updateSort($this->editor, $ctrl); // editor
            $this->updateSort($this->kurator, $ctrl); // kurator
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fphp_kurasi_loloslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fphp_kurasi_loloslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fphp_kurasi_loloslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->produk_foto->UseColorbox) {
                ...
                $this->produk_foto->LinkAttrs["data-rel"] = "php_kurasi_lolos_x" . $this->RowCount . "_produk_foto";
            ...
            if ($this->produk_foto_1->UseColorbox) {
                ...
                $this->produk_foto_1->LinkAttrs["data-rel"] = "php_kurasi_lolos_x" . $this->RowCount . "_produk_foto_1";
            ...
            if ($this->produk_foto_2->UseColorbox) {
                ...
                $this->produk_foto_2->LinkAttrs["data-rel"] = "php_kurasi_lolos_x" . $this->RowCount . "_produk_foto_2";
            ...
            if ($this->produk_foto_3->UseColorbox) {
                ...
                $this->produk_foto_3->LinkAttrs["data-rel"] = "php_kurasi_lolos_x" . $this->RowCount . "_produk_foto_3";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fphp_kurasi_loloslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fphp_kurasi_loloslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fphp_kurasi_loloslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_php_kurasi_lolos" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_php_kurasi_lolos\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fphp_kurasi_loloslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fphp_kurasi_loloslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
