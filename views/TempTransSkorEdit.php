...
// Page object
$TempTransSkorEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var ftemp_trans_skoredit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    ftemp_trans_skoredit = currentForm = new ew.Form("ftemp_trans_skoredit", "edit");
    ...
    ftemp_trans_skoredit.addFields([
    ...
    // Validate form
    ftemp_trans_skoredit.validate = function () {
    ...
    // Form_CustomValidate
    ftemp_trans_skoredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ftemp_trans_skoredit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("ftemp_trans_skoredit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="ftemp_trans_skoredit" id="ftemp_trans_skoredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="temp_trans_skor" data-field="x_nik" data-hidden="1" name="o_nik" id="o_nik" value="<?= HtmlEncode($Page->nik->OldValue ?? $Page->nik->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="temp_trans_skor" data-field="x_jenis_nilai" data-hidden="1" name="o_jenis_nilai" id="o_jenis_nilai" value="<?= HtmlEncode($Page->jenis_nilai->OldValue ?? $Page->jenis_nilai->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
