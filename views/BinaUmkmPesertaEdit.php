...
// Page object
$BinaUmkmPesertaEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fbina_umkm_pesertaedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fbina_umkm_pesertaedit = currentForm = new ew.Form("fbina_umkm_pesertaedit", "edit");
    ...
    fbina_umkm_pesertaedit.addFields([
    ...
    // Validate form
    fbina_umkm_pesertaedit.validate = function () {
    ...
    // Form_CustomValidate
    fbina_umkm_pesertaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbina_umkm_pesertaedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fbina_umkm_pesertaedit.lists.id_binadata = <?= $Page->id_binadata->toClientList($Page) ?>;
    loadjs.done("fbina_umkm_pesertaedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fbina_umkm_pesertaedit" id="fbina_umkm_pesertaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_bina_umkm_peserta_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_bina_umkm_peserta_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="bina_umkm_peserta" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fbina_umkm_pesertaedit"], function() {
    fbina_umkm_pesertaedit.createAutoSuggest(Object.assign({"id":"x_id_binadata","forceSelect":false}, ew.vars.tables.bina_umkm_peserta.fields.id_binadata.autoSuggestOptions));
});
</script>
