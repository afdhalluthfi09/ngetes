...
// Page object
$BinaUmkmPesertaCopyEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fbina_umkm_peserta_copyedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fbina_umkm_peserta_copyedit = currentForm = new ew.Form("fbina_umkm_peserta_copyedit", "edit");
    ...
    fbina_umkm_peserta_copyedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fbina_umkm_peserta_copyedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    fbina_umkm_peserta_copyedit.validate = function () {
    ...
    // Form_CustomValidate
    fbina_umkm_peserta_copyedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbina_umkm_peserta_copyedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fbina_umkm_peserta_copyedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fbina_umkm_peserta_copyedit" id="fbina_umkm_peserta_copyedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_bina_umkm_peserta_copy_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_bina_umkm_peserta_copy_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="bina_umkm_peserta_copy" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
